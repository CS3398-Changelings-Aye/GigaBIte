...
* Nisa Lateef: Uploaded the images.py file that allows the user to search for images of specific ingredients they would like. Also attempted to make save recipe function that can be used in the discord bot. This file wasn't functional (one of the roadblocks), which is why it isn't in cogs, however it can be viewed in the Nisa branch in the ingridients.py file. 
